...
def step_size(s):
    ...
    """
    step_size = STEP_SIZE_CONSTANT - hash_word(s, STEP_SIZE_CONSTANT)
    return step_size
...
def main():
    ...
    length = (length * 2)
    while is_prime(length) is False:
        length += 1
    ...
    length_m = int(0.2 * len(word_list))
    while is_prime(length_m) is False:
        length_m += 1
    print(length_m)
    hash_memo = []
    ...
    for word in word_list:
        if is_reducible(word, word_list, hash_memo):
            reducible_words.append(word)
    # find the largest reducible words in reducible_words
    long_words = get_longest_words(reducible_words)
